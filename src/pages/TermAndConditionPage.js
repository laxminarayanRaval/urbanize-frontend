...
const contentsData = [
  ...
  whether personally or on behalf of an entity (“you”) and Urbanize Solutions LLC
  ("Company," “we," “us," or “our”), concerning your access to and use of the
  https://urbanize-frontend.herokuapp.com/ website as well as any other media form,
  ...
  registered office at Airport Road, Surat, Gujarat 395001. You agree that by accessing
...
enterprise.`,
  ...
  {
    header: "USER GENERATED CONTRIBUTIONS",
    content: [
...
and/or distribute all content relating to reviews.`,
  ...
  {
    header: "MOBILE APPLICATION LICENSE",
    content: [
      `Use License`,
...
with applicable third-party terms of agreement when using the mobile application,e.g., if you have a VoIP application, then you must not be in violation of their wireless
...
contained in these Terms of Use against you as a third-party beneficiary thereof.`,
  ...
  Please be advised the Site is hosted in India. If you access the Site from any other
  ...
  your continued use of the Site, you are transferring your data to India, and you agree
  to have your data transferred to and processed in India.`,
    ],
  },
  {
    header: "COPYRIGHT INFRINGEMENTS",
    content: [
  ...
  WITHOUT WARNING, IN OUR SOLE DISCRETION.If we terminate or suspend your account for any reason, you are prohibited from
  ...
  {
    header: "MODIFICATIONS AND INTERRUPTIONS",
    content: [
  ...
  },
  {
    ...
    content: [
      `These Terms shall be governed by and defined following the laws of India. Urbanize
  Solutions LLC and yourself irrevocably consent that the courts of India shall have
  ...
  number of arbitrators shall be three (3). The seat, or legal place, of arbitration shall
  be Surat, Gujarat, India. The language of the proceedings shall be English. The
  ...
  other information. We reserve the right to correct any errors, inaccuracies, oromissions and to change or update the information on the Site at any time, without
  prior notice.`,
    ],
  },
  {
  ...
  all data that you transmit or that relates to any activity you have undertaken using theSite. You agree that we shall have no liability to you for any loss or corruption of any
  such data, and you hereby waive any right of action against us arising from any such
  loss or corruption of such data.`,
    ],
  },
  {
    header: "ELECTRONIC COMMUNICATIONS, TRANSACTIONS, AND SIGNATURES",
    content: [
  ...
  {
    header: "CALIFORNIA USERS AND RESIDENTS",
    content: [
...
const TermAndConditionPage = () => {
  ...
  return (
    ...
    >
      ...
      <Typography variant="subheader">Last updated July 13, 2022</Typography>
      <Divider sx={{ width: "75%" }} />
      <Typography variant="h5">TABLE OF CONTENTS</Typography>
      <List>
        {contentsData.map((data, index) => (
          // <ListSubheader key={index * 2 + 3}>
          <ListItem key={index * 2 + 3}>
            <Typography
              component={Link}
              href={"#" + makeSlug(data?.header)}
              variant="body2"
            >{`${index + 1} : ${data?.header}`}</Typography>
          </ListItem>
          // </ListSubheader>
        ))}
      </List>
      ...
      <List sx={{ maxWidth: "75%" }}>
        {contentsData.map((data, index) => (
          <>
            <ListSubheader id={makeSlug(data?.header)} key={index * 2 + 3}>
              <Typography variant="h5">{`${index + 1} : ${
                data?.header
              }`}</Typography>
            </ListSubheader>
            {data?.content.map((p, i) => (
              <ListItem key={`item-${index * i}-${i * 1}`}>
      ...
      <pre>
        <b>Urbanize Solutions LLC</b><br />
        <i>Airport Road</i><br />
        <i>Surat, Gujarat 395000,India</i><br />
        <i>Phone: __________</i><br />
        <i>lx.raval01+admin.urbanize@gmail.com</i>
      </pre>
    </Box>
  );
};
